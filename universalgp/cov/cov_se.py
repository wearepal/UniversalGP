...
class SquaredExponential:
    ...
    def __init__(self, input_dim, ell=1.0, sf=1.0, iso_ard=False):

        # if iso_ard = False, consider ARD (automatic relevance determination) kernel,
        # else, consider ISO (isotropic ) kernel

        if iso_ard:
            self.ell = tf.Variable([ell], dtype=tf.float32)
        else:
            self.ell = tf.Variable(ell * tf.ones([input_dim]))
