...
class VariationalSensitive:
    ...
    def inference(self, features, train_outputs, is_train):
        ...
        Returns:
            negative evidence lower bound with respect to sensitive attributes and variables to train
        """
        ...
        sensi_attr = features['sensitive']
        train_inputs = features['input']

        sensitive_ell = [0 for _ in range(self.args['num_sensitive'])]
        for i in range(self.args['num_sensitive']):
            # sensitive_inputs[i] = tf.gather_nd(inputs, tf.where(tf.logical_and(tf.equal(sensi_attr, 0), tf.equal(train_outputs, 1))))
            sensitive_train_inputs = tf.gather_nd(train_inputs, tf.where(tf.equal(sensi_attr, i)))
            sensitive_train_outputs = tf.gather_nd(train_outputs, tf.where(tf.equal(sensi_attr, i)))
            sensitive_ell[i] = self._build_ell(weights, self.means, chol_covars, self.inducing_inputs, kernel_chol,
                                               sensitive_train_inputs, sensitive_train_outputs)
        ell_diff_sum = 0
        index = 0
        for i in range(self.args['num_sensitive']):
            for j in range(i+1, self.args['num_sensitive']):
                # ell_diff_sum += self.smooth_abs(sensitive_ell[i] - sensitive_ell[j], 0.1)
                ell_diff_sum += tf.abs(sensitive_ell[i] - sensitive_ell[j])
                index += 1

        batch_size = tf.to_float(tf.shape(train_inputs)[0])
        nelbo = -((batch_size / self.num_train) * (entropy + cross_ent) + tf.reduce_sum(sensitive_ell)) \
                + PENALTY * tf.abs(ell_diff_sum - index * EPISILON)
    ...
    @staticmethod
    def smooth_abs(x, precision):
        return precision * tf.log(2 * tf.cosh(x / precision))
